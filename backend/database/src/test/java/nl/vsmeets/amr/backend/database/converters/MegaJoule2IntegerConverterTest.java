...
class MegaJoule2IntegerConverterTest {
  ...
  void setUp(@Mock final ApplicationContext applicationContext) throws Exception {
    testObject = new MegaJoule2IntegerConverter();
    ...
    lenient().when(applicationContext.getBean("megaJoule", Unit.class)).then(i -> megaJoule);
  ...
  void testConvertToDatabaseColumn(@Mock final Quantity<Energy> energy) {
    ...
    when(energy.to(megaJoule)).then(i -> energy);
  ...
  void testConvertToEntityAttribute(@Mock final Quantity<Energy> convertToEntityAttribute) {
    ...
    when(energyQuantityFactory.create(value, megaJoule)).then(i -> convertToEntityAttribute);
